...
We have build a model to represent the latency of 5G MEC (UE to gNB) and URLLC (the full path from UE to Cloud).
...
In order to make the delay less than 1 ms, we set service rate from 45Gbit/s to 48Gbit/s based on arrival rate is 20Gbit/s.
...
According to the architecture characteristics, the URLLC network is modeled as a tandem system which demonstrate the communication rom UE to Cloud.
