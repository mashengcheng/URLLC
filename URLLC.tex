...
%Utilizing the relationship between the factors and delay can provide valuable guidelines for the early design of URLLC architecture.
...
The SNC is a continuous development method to analyze network traffic characteristic and evaluate performance\cite{b4}.
...
%在这里加上我的理论分析，对于今后的网络部署，如何减小时延提供了指导意义，已经优化方向
...
%and obtain the relationship between delay and some key factors.
...
%The main factors that affect the delay are be derived.
...
%A way to offer URLLC without intervention in the baseband/PHY layer design is to use interface diversity and integrate multiple communication interfaces
...
Jimmy and his colleagues propose an analysis framework that combines traditional reliability models with technology-specific latency probability distributions\cite{interface}.
...
\section{Numerical Result and Performance Evaluation}
%考虑4G和5G的时延对比？通过参数来体现？
%服务曲线，要带有5G网络的特性的

In this section, we shall discuss what factors are the main cause of latency in URLLC.

According to the delay requirement of URLLC, we write a simulator to validate our analysis result.
We implement the simulator in Python 3.6. It can calculate the delay value in accordance with the input network parameters.

To validate the correctness of analysis, we set the violation probability from $1\times10^{-6}$ to $1\times10^{-4}$.
We compare the analysis with the simulator, and the Figure.\ref{validation} shows the result.
\begin{figure}[htbp]
  \centering
  \includegraphics[width=.5\textwidth]{validation.png}
  \caption{Comparison between Simulator and Analysis}
  \label{validation}
\end{figure}
Figure.\ref{validation} shows a good match between the analytical data and the corresponding simulation result.
Especially, the analytical result is also the boundary of the simulation data.
The boundary is slightly loose when the value of violation probability is large, while it is more tighter when value is little.
This fully illustrates the correctness of the analysis results.
...
%那次和李老师一起上去讨论买设备的时候，周金和老师说的分片，其实就是云资源的再分配，这里可以是大数据的概念，怎么分配才能满足性能的需要，同时还能保证资源消耗最小。分配可以是预测的，这里就可以用到ML的方法。同时保证资源消耗最小，这里可以是优化问题。
