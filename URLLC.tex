...
According to the requirement of URLLC, the reliability and latency can be described as Formula.\ref{eq_urllc_delay_problem}:
...
If each server nodes $n(=1,2,...,N)$ provides a stochastic service curve $S^{n}~<g^{n}, \beta^{n}>$ to its input, then the network guarantees to the flow a stochastic service curve $S~<g,\beta>$ with
\begin{align*}
...
\end{align*}
With stochastic arrival curve (\ref{def_SAC}), $A_{AAU}(s,t)-\alpha(t-s)$ is bounded by $f_{AAU}(x)$.
According to stochastic service curve (\ref{def_SSC}), $A_{AAU}\otimes \beta_{All}(t+d)-D_{Cloud}(t+d)$ is limited by $g_{All}$.
For a long time running, if $t\rightarrow\infty$, $\alpha_{AAU}(t-s)-\beta_{All}(t+d-s)$ approximate to zero because of $\alpha_{AAU},\beta_{All}\in\mathcal{F}$.
