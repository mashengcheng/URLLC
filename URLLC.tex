...
So the whole delay or latency in 5G system is contribute by the time processing of RAN, fronthual, backhaul, Core Network and Cloud server. It can be expressed as Formula.\ref{eq_delay_composition}.
...
The Formula.\ref{eq_urllc_problem} represents the 5G URLLC network successfully transport data and satisfy the delay and reliability requirements.
...
\end{equation}
\end{corollary}
where $\beta_{gNB}(t)=\beta_{AAU}\otimes \beta_{DU}\otimes \beta_{CU}(t)$, and $g_{gNB}=g_{AAU}\otimes g_{DU}\otimes g_{CU}(x)$.

$Proof:$ Please see Appendix A.
%rewrite this part.

%这里不应该是根据串联特性的文献了，应该是前面提到的定理了。
%这部分，先不弄，对于这部分的想法是，先把gNB的内容都分析完了，再扩展一下，就是整个的了，就可以加上CN和cloud了。我现在先把gNB都分析完。
...
We use $h(\alpha+x, \beta)$ replace the $d$. where $h(\alpha+x,\beta)$ is the maximum horizontal distance between $\alpha+x$ and $\beta$ for $x \geq 0$. The $h(\alpha,\beta)$ function implies the condition
...
&\leqslant P\{\{A_{AAU}(t) - D_{CU}(t+h(\alpha+x, \beta))\} > 0\}  \\
&\leqslant P\{\sup_{0\leqslant s \leqslant t}\{A_{AAU}(s,t)-\alpha_{AAU}(t-s)\}  \\
