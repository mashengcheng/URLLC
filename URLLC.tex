...
Yuming Jiang's research field is relatively extensive, including theory and application \cite{jiang_power}\cite{jiang_On}\cite{jiang_theory}\cite{jiang_Wireless}.
...
\begin{equation}
\begin{split}
g_{All}(x)&=\inf_{x_{1}+x_{2}+x_{3}+x_{4}+x_{5}=x}\sum_{k=1}^{5}a_{k}e^{-b_{k}x_{k}} \\
...
&\leq\inf\{e^{x-(\lambda t+x)ln\frac{\lambda t+x}{\lambda t}}+ e^{\frac{-xb}{n+1}}(5(n+1))\}
\end{split}
\end{equation}
...
\bibitem{jiang_On} Wu K, Jiang Y, Li J. On the model transform in stochastic network calculus[C]// International Workshop on Quality of Service. IEEE, 2010:1-9.
