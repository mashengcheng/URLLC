...
%用户端发送的流量可以用随机到达过程来描述,而网络节点提供的服务能力用服务过程来描述
The network traffic of URLLC can be described by stochastic processes as introducing in above. The data sent by UEs can be described as arrival process $A(t)$ and the service capacity provided by the network server node can be represented by stochastic service process $S(t)$.
