...
In stochastic service curve \ref{def_SSC}, $A\otimes\beta$ represent the flow with lossless situation. When we take into account the loss in network system, $A\otimes\beta\oslash\gamma$ denotes the service capacity after considering loss or error. 
As defined in \ref{eq_sic}, $\gamma$ is the upper bound for error arising. If the value of $gamma$ is high, it means the running state of system is bad and error occurs very often. 
...
We mainly focus on the errors from processing and forwarding of server in URLLC. 
So we present a novel stochastic loss process which work on server node.
...
\bibitem{article_WLE}
Guly¨¢s, Andr¨¢s, B¨ªr¨®, J¨®zsef, A stochastic extension of network calculus for workload loss examinations, Communications Letters IEEE, \textbf{10}(5), pp.399--401 (2006)
\bibitem{proc_Loss}
