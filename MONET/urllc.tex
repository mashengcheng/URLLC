...
\title{Applying Stochastic Network Calculus to Analyze Delay of URLLC in 5G\thanks{Supported by program National Natural Science Foundation of China. (Nos.61370065,61502040).Beijing Municipal Program for Excellent Teacher Promotion. (No.PXM2017\_014224.000028).}}
...
Little efforts have been made on applying a theoretical method to analyze the delay bound.
...
%这部分内容是error和loss部分的相关工作，可以放在前面来引出。要真么引出呢？我们说，考虑URLLC的可靠性的时候，原版的SNC理论并没有对这部分的研究
%一些研究者考虑从loss的角度来体现系统的可靠性。然后说下面的这一堆，
...
A novel stochastic curve which work on server node will be proposed.

%这里引出了如何考虑urllc的可靠性。 为了说明可靠性，那么就要用impairment过程来表示BLER。
%impairment表示系统在s到t时段产生的错误块，
...
We denote the stochastic impairment process I(\tau) as the block error rate occurred in the server.
\begin{definition}\label{def_SIC}
{\bfseries(Stochastic Impairment Curve)}.
A system is said to have a stochastic impairment curve $\gamma\in\mathcal{F}$ with bounding function $h \in \mathcal{\bar{F}}$, denoted by $I \sim<h, \gamma>$, if for all $t\geq0$ there holds
\begin{equation}\label{eq_sic}
P\Bigl\{\sup_{0\leq s \leq t}[I(s,t) - \gamma(t-s)] > x \Bigr\} \leq h(x).
\end{equation}
\end{definition}
The (\ref{eq_sic}) means that the system server processes the received block and probably checkouts erroneous data.
The process $I(s,t)$ represents the error or loss generated from time $s$ to $t$.
The curve $\gamma$ is the upper bound for the impairment process $I$.
The impairment process $I$ is usually less than the impairment curve $\gamma$.
However, we assume that the impairment curve $\gamma(\tau)$ may be exceeded by impairment process $I(\tau)$ in sometimes,
...
When we take into account the loss in network system, $A\otimes\beta\oslash\gamma$ denotes the service capacity after considering loss or error.
As defined in (\ref{eq_sic}), $\gamma$ is the upper bound for error arising.
If the value of $\gamma$ is high, it means the running state of system is unstable and error occurs very often.
In order to express the ultra reliability of the communication, we can set the $\gamma$ value as the BLER.
The impairment process will be
\begin{equation}
P\{I(s,t)>x\}\leq h(x)
\end{equation}
Because of our scenario is URLLC, it is ultra reliable communication.
...
With Corollary.\ref{crl_latency_urllc}, we know that 4 key variance need to be determined.
...
According to the architecture characteristics, the URLLC network is modeled as a tandem system which demonstrates the communication from UE to Cloud.
...
Numerical results demonstrate that the main factor which can impact on latency is the number of servers deployed in tandem.
...
\bibitem{standard3}
 3GPP TS 36.212, Evolved Universal Terrestrial Radio Access (E-UTRA); Multiplexing and channel coding (2018)
\bibitem{article1}
...
R. Lübben and M. Fidler, Estimation method for the delay performance of closed-loop flow control with application to TCP, IEEE INFOCOM 2016-The 35th Annual IEEE International Conference on Computer Communications, pp.1--9 (2016)
...
R. Lübben, M. Fidler and J. Liebeherr, Stochastic Bandwidth Estimation in Networks With Random Service, IEEE/ACM Transactions on Networking, \textbf{22}(2), pp.484--497 (2014)
...
\bibitem{article_WLE}
Gulyás, András, Bíró, József, A stochastic extension of network calculus for workload loss examinations, Communications Letters IEEE, \textbf{10}(5), pp.399--401 (2006)
\bibitem{proc_Loss}
