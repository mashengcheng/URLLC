...
%Under a suitable traffic model and a chosen server model,
...
2) Stochastic latency bound for URLLC network is obtained.
The analysis method of delay bound also supports the communication in mobile edge computing.
To analyze the reliability of the network, we provide a stochastic error process which can express the block error rate.
We also investigate the impacts of block error rate on latency.

3) %Our analysis results represent which parameters are the key factors affecting the delay.
%By adjusting the key factors, we give some strategies which can reduce the delay effectively.
Based on our analysis results, we can find out what parameters are the key factors affecting the delay.
% how to reduce the delay effectively.
...
According to the architecture characteristics, the URLLC network is modeled as a tandem system which discribes the communication from UE to Cloud.
Applying stochastic network theory and combining the features of URLLC network, performance analysis has been conducted.
We have investigated the relationship between delay constraint, service rate, violation probability and the number of deployed servers in URLLC network.
...
With the increase of service rate, delay can be reduced.
...
%With the further development of the researches
