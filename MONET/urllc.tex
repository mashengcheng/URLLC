...
Because of $\gamma$ is the ceiling of the process $I$, so we can get
\begin{equation}\label{mgf_gamma}
E[e^{\theta I(s,t)}] \leq e^{\theta \gamma(t-s)}
\end{equation}
Then we put (\ref{mgf_gamma}) into right hand of (\ref{chernoff_err}), and we hold
\begin{equation}\label{chernoff_bound}
\begin{split}
P\{I(s,t)\geq x\} &\leq e^{-\theta x}\cdot e^{\theta \gamma(t-s)} \\
&\leq e^{-\theta x + \theta\gamma(t-s)}  \\
&\leq e^{\theta(\gamma(t-s)-x)}
\end{split}
\end{equation}
