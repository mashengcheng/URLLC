...
To study the reliability of URLLC, we propose a new process to describe the error rate and packet loss rate.
We denote the impairment process by $I\sim<h,\gamma>$.
It represents the error and loss happened in the server.
...
\end{equation}
\end{definition}
The (\ref{eq_sic}) means that the system server probably make loss or error.
...
%在这里，把那两篇论文也加上去，作为引用。
%关于损失过程的描述，文献【1】【2】都对这个问题进行了分析。
\subsection{write at here}
In term of loss or impairment analysis in stochastic network calculus, article \cite{article_WLE} and \cite{proc_Loss} are both representative research on loss process.
A. Gulyas and J. Biro propose a Workload Loss Ratio Estimation to express the loss in the network flow.
An effective w-arrival curve $Z^{\varphi}$ was put forward in this paper.
The loss was considered in arrival process which is the beginning of the transmission.
And they use $Z^{\varphi}\oslash S^{\varphi_{s}}$ to present the loss generated in service process.
...
In paper \cite{proc_Loss}, the authors gave a stochastic loss bound by MGF. 

We mainly focus on the errors server processing and forwarding of server in URLLC.
%这两句说的不是很好。

The impairment process must work with the stochastic service process.
