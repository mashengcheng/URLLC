...
3) Based on our analysis method, we obtain the relationship between Delay and Arrival Rate and Service Rate.
We also calculate the minimum system service rate that meets the delay requirement of URLLC.
Further, our analysis reveals that the system delay can be reduced by decreasing number of tandem servers.
The result provides guidelines for deployment to meet stringent delay requirements.
...
In terms of resource allocation and energy efficiency, there is also some research on URLLC.
...
\begin{figure*}[htbp]
\centering
  \includegraphics[width=0.75\textwidth]{5G_delay_en.eps}
...
That is because the delay requirement of the user plane is higher than that of control plane.
...
Equation(\ref{eq_urllc_delay_problem}) represents that a 5G URLLC network successfully transfers data and meets the delay requirements.
...
\begin{split}
P\{L_{All}(t) \geq d\} &= P\{L_{All}(t) \geq h(\alpha_{AAU}+x, \beta_{All})\} \\
...
P\{L(x)>h(\alpha_{AAU}+x, \beta_{All})\} < f_{AAU} \otimes g_{All}(x)
\end{equation}
Therefore, Theorem. \ref{crl_latency_urllc} is proved.
\end{proof}
...
More simulation parameters can be found in Tab. \ref{tab_2}.
\begin{table}
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=.7\textwidth]{delay_no_tandem.eps}
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.7\textwidth]{BLER.eps}
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=.7\textwidth]{Delay_BLER.eps}
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=.7\textwidth]{Delay_Arrival.eps}
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=.7\textwidth]{delay_servers.eps}
...
It means that when the number of servers is less than 5 and the service rate is over 40 Gbit/s, the delay maintains below 1 ms.
In order to ensure low latency of communication, it is necessary to reduce the number of tandem servers in the deployment.
...
The simulation experiment of the whole paper mainly aims at the performance requirements of URLLC.
...
Our experiment reveals how to meet the requirements of latency and reliability.
To increase the transmission rate of the network to 20GBit/s, we need to meet the conditions in network construction.
