...
\title{Performance evaluation of URLLC in 5G based on Stochastic Network Calculus\thanks{Supported by program National Natural Science Foundation of China. (Nos.61502040).Beijing Municipal Program for Excellent Teacher Promotion. (No.PXM2017\_014224.000028).}}
...
Little effort has been made on applying a theoretical method to analyze the delay bound.
...
A tandem model of the generation of delay in 5G network is built, and parameters which have influence on the delay are analyzed 
%Optimizing these factors to reduce the delay will provide valuable guidelines for the early design of URLLC architecture.
We investigate how to optimize the parameters to reduce the delay, which will provide valuable guidelines for the design of URLLC architecture.
...
2) Our analysis results represent which parameters are the key factors affecting the delay. 
%By adjusting the key factors, we give some strategies which can reduce the delay effectively.
Based on our analysis results, we can find how to reduce the delay effectively.
...
%下面要做的事情就是用chernoff边界，来计算一个gamma的影响
...
%Because of $\gamma$ is the ceiling of the process $I$, and we so we can get
...
C = \hat{C}(1-\gamma)
\end{equation}
where $\hat{C}$ is the service capacity without loss and error, and the $C$ is the adjusted service rate.
...
%to include the demand value $1*10^{-5}$
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=.5\textwidth]{BLER.png}
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=.5\textwidth]{Delay_BLER.png}
  \centering\caption{Delay with Different BLER }
  \label{fig_Delay_BLER}
\end{figure}
%end the figure
%To study the relationship between delay and reliability,
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=.5\textwidth]{Delay_Arrival.png}
